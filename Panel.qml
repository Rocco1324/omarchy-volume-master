...
Panel {
  id: root
  moduleName: "community.volume-master-in-omarchy"
  ipcTarget: "community.volume-master-in-omarchy"
  ...
  readonly property string barVolumeText: {
    var percent = Math.round(outputVolume * 100)
    return percent > 100 ? percent + "% (+" + (percent - 100) + "%)" : percent + "%"
  }
  ...
  implicitWidth: bar && bar.vertical ? button.implicitWidth : barRow.implicitWidth
  ...
  function startProcessWithDeadline(proc) {
    processKillTimer.running = false
  ...
  Timer {
    ...
    onTriggered: {
      if (!sinkAvailabilityProc.running) startProcessWithDeadline(sinkAvailabilityProc)
    }
  }
  ...
  Timer {
    ...
    triggeredOnStart: true
    onTriggered: {
      if (!volumeSinkProc.running) startProcessWithDeadline(volumeSinkProc)
    }
  }
  ...
  Row {
    ...
    WidgetButton {
      ...
      bar: root.bar
      text: root.barVolumeText
      fontSize: Style.font.bodySmall
      horizontalMargin: 2
      active: root.outputVolume > 1
      tooltipText: "Output volume: " + root.barVolumeText
      onPressed: function(b) {
        if (b === Qt.RightButton) root.toggleAllMuted()
        else root.toggle()
      }
          ...
          Column {
            ...
            Item {
              ...
              Text {
                ...
                textFormat: Text.PlainText
                text: {
                  var percent = Math.round((outputSlider.dragging ? outputSlider.liveValue : root.outputVolume) * 100)
                  return percent > 100 ? percent + "% (+" + (percent - 100) + "%)" : percent + "%"
                }
                color: Qt.darker(root.bar.foreground, 1.4)
                font.family: root.bar.fontFamily
